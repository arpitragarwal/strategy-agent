...
function buildQuantEnumMarkdown(): string {
  ...
  lines.push("");
  lines.push(
    `- **Numeric enums** — \`contract_term_years\` on **crm/deal_data** / **crm/accounts**: integers ${PROTOTYPE_CONTRACT_TERM_YEARS.join(", ")}; **crm/accounts.logo_acquisition_cohort** and **crm/deal_data.logo_acquisition_cohort** (same value per \`account_id\` as accounts; calendar year cohort key in the prototype) are numeric (not string \`eq\`).`,
  );
  lines.push("");
  return lines.join("\n");
}
